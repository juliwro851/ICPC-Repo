...
# -*- coding: utf-8 -*-
"""
train_heterogeneous_rllib.py

Heterogeneous multi-agent training using RLlib PPO.
- Supports both homogeneous (all agents same) and heterogeneous (different roles) modes
- In heterogeneous mode: rescuers and firefighters have different action spaces and specialize
- Proper multi-agent policy configuration with role-based learning
- Saves training metrics, episode statistics, and creates visualization GIFs
"""
...
class HeterogeneousCallback(DefaultCallbacks):
    """Callback to collect detailed episode statistics for heterogeneous training."""
...
def main():
    ...
    for i in range(args.iters):
        ...
        if (i + 1) % max(1, args.iters // 5) == 0 or i == args.iters - 1:  
            ...
            obs, infos = viz_env.reset(options={"iter_idx": i, "episode_idx": 1})
            ...
            for step in range(env_config["max_steps"]):
                ...
                obs, rewards, dones, truncs, infos = viz_env.step(actions)
