...
class SARParallelEnv(ParallelEnv):
    ...
    def render_array(self):
        """Zwraca RGB obraz planszy - identyczny dla obu trybów"""
        ...
        if self.heterogeneous:
            ...
            # jednolite kolory dla trybu homogenicznego
    ...
    def step(self, actions):
        ...
        for agent, local_action in actions.items():
            ...
            if act in [ACT_UP, ACT_DOWN, ACT_LEFT, ACT_RIGHT]:
                ...
                if self.heterogeneous:
                    efficiency = 2 if agent_role == ROLE_FIREFIGHTER else 1
                else:
                    efficiency = 1
    ...
    def _make_obs_for(self, agent):
        """Obserwacje - różnią się w zależności od trybu"""
        r = self.obs_radius
        H = W = 2 * r + 1
        x, y = self.agent_positions[agent]
        ...
        base_obs = np.stack([patch, victims, agents], axis=0).reshape(-1).astype(np.float32)
